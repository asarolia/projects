...
namespace RunBoard
{
    public partial class datepage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            seldate1.Text = Convert.ToString(System.Web.HttpContext.Current.Session["date"]);
        ...
        private void getHalerrStatus()
        {
            ...
            //Chart16.Series["Series1"].Points[0].Color = Color.Gray;
        ...
        private void getBatchStatus()
        {
            ...
            int cntr = 0;
            //  SqlDataReader myReader = null;
            String sqlst = "SELECT Job_Category FROM BatchData WHERE Jobname IN ('PXACC048','PXACC050') AND RecordDt = @ddate";
               ...
               // objCommand.Parameters.AddWithValue("@name", "PXACC048");
                ...
                if (myReader.HasRows)
                {
                    while (myReader.Read())
                    {
                        cntr = cntr + 1;
                        // Check for Amber flag
                        if ((myReader.GetString(0).Trim() == "amber"))
                        {
                        ...
                        if ((myReader.GetString(0).Trim() == "red"))
                        {
                            //   tiflag = 0;
                            Chart13.Series["Series1"].Points.DataBindY(yValue);
